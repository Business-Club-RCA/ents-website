...
export function HeroVisual({ announcement }: HeroVisualProps) {
  ...
  return (
    <section className="relative w-full min-h-[92vh] sm:min-h-screen flex flex-col overflow-hidden bg-[#0e0e11] text-white select-none">
      ...
      <div className="relative z-10 flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col justify-end pb-10 sm:pb-14 lg:pb-0">

        {/* Headline + Subtitle + CTAs — sits above the bottom panel */}
        <div className="pb-8 sm:pb-12 lg:pb-16 max-w-3xl space-y-5 sm:space-y-6">

          {/* Kicker Badge — solid, clearly visible */}
        ...
        {/* ── ANNOUNCEMENT PANEL — pinned at the very bottom of the hero ── */}
        {announcement && announcementSlug && (
          <div className="animate-hero-cta relative border-t border-white/[0.12] pt-5 pb-8 sm:pb-10 lg:pb-12">
            {/* Subtle left accent rule */}
            <div className="absolute left-0 top-5 bottom-0 w-px bg-gradient-to-b from-emerald-400/60 via-emerald-400/20 to-transparent" />

            <div className="pl-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-5 sm:gap-8">
              ...
              <div className="flex-1 min-w-0 space-y-2">
                ...
                <Link href={`/updates/${announcementSlug}`} className="block group/title">
                  <h3 className="text-lg sm:text-xl lg:text-2xl font-bold text-white leading-snug line-clamp-1 group-hover/title:text-neutral-200 transition-colors">
                    {announcement.title}
                  </h3>
                  <p className="mt-1 text-sm text-neutral-400 line-clamp-1 leading-relaxed font-normal">
